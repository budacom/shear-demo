class DrivingLicenceStencil < BaseStencil
  ...
  def process_match
    ...
    @surnames = read_relative_if_possible(:surnames, [48.601, 30.3], [43.268, 32.239],
  ...
  def read_relative_if_possible(_label, _min, _max,
    ...
    exclusion = _exclusion_key.present? ? self.class.template.get_exclusions(_exclusion_key) : Set[]

    if match.labels.include? _label
      reference_location = self.class.template.fixtures.find { |fixture| fixture[2] == _label }[1]
      ...
      match.read_relative(_label, min_relative, max_relative,
        line_height: _line_height, exclusion: exclusion, _delete: delete)
    else
