module ImageProcessingUtils
    ...
    _new_image.each_with_index do |row, y|
      row.each_with_index do |pixel, x|
        ...
        if inside?(position, _image)
          target_pixel = _image[position[y]][position[x]]
  ...
  def inside?(_position, _image)
    ...
    return false if position[0].negative?

    return false if position[0] >= height

    return false if position[1].negative?

    return false if position[1] >= width
